...
export default function PageLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="bg-[#FEF9F3] text-[#1C1A17] font-sans selection:bg-[#9A4100]/20 min-h-screen flex flex-col">
          ...
          <div className="hidden md:flex items-center space-x-12">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className={`transition-colors text-xs font-bold uppercase tracking-[0.2em] ${
                  isHome
      ...
      <footer className="w-full bg-[#F2EDE7] border-t border-[#8a7266]/10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-16 px-12 py-24 max-w-[1536px] mx-auto">
          <div className="md:col-span-5 space-y-8">
            <Link href="/" className="text-3xl font-serif font-bold text-[#9A4100] block">Fork &amp; Compass</Link>
            ...
            <div className="space-y-6">
              ...
              <ul className="space-y-4 text-base text-[#725a3c]">
                {FOOTER_DISCOVER.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="hover:text-[#9A4100] transition-colors">{link.label}</Link>
                  </li>
                ))}
              </ul>
            </div>
